...
src(o0)
.layer(
  shape(2, 0.01, 0).r().color(0,0,0).modulateRotate(osc(.05, 3), Math.PI * 1.3)
) // esse primeiro layer é o macete
 .layer(
   shape([3, 4, 3], .2, 0).r().mult(osc(4, 2, 22.1).contrast(1))
     .modulate(
       //osc(1,1,Math.PI/2).brightness(-.5)
      noise(3, .05).pixelate(5,5)
     .color(1, 0), 1)
     .modulateRotate(osc(1, 1).pixelate(1, 1).brightness(-.5), Math.PI * 2
